...
// Type aliases matching libdragon names so ported engine code compiles with minimal changes
using fm_vec2_t = glm::vec2;
using fm_vec3_t = glm::vec3;
using fm_quat_t = glm::quat;
using fm_mat4_t = glm::mat4;

// Inline compatibility shims for libdragon/tiny3d math functions used in collision code
inline float fm_vec2_dot(const fm_vec2_t &a, const fm_vec2_t &b) {
  return glm::dot(a, b);
}

inline void fm_vec3_cross(fm_vec3_t *res, const fm_vec3_t *a, const fm_vec3_t *b) {
  *res = glm::cross(*a, *b);
}

inline void fm_vec3_norm(fm_vec3_t *res, const fm_vec3_t *v) {
  float len = glm::length(*v);
  *res = (len > 0.0f) ? (*v / len) : fm_vec3_t{0.0f};
}

inline void fm_vec3_sub(fm_vec3_t *res, const fm_vec3_t *a, const fm_vec3_t *b) {
  *res = *a - *b;
}

inline float t3d_vec3_dot(const fm_vec3_t *a, const fm_vec3_t *b) {
  return glm::dot(*a, *b);
}

inline float t3d_vec3_len(const fm_vec3_t *v) {
  return glm::length(*v);
}

inline float t3d_vec3_len2(const fm_vec3_t *v) {
  return glm::length2(*v);
}

inline float t3d_vec3_distance2(const fm_vec3_t *a, const fm_vec3_t *b) {
  return glm::distance2(*a, *b);
}

inline fm_quat_t fm_quat_from_axis_angle(const fm_vec3_t &axis, float angle) {
  return glm::angleAxis(angle, axis);
}

inline fm_quat_t fm_quat_inverse(const fm_quat_t &q) {
